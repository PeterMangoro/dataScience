#!/usr/bin/env python
# coding: utf-8
"""GDP Assignment 3 — full analysis pipeline .

Run from anywhere: ``python assignment3.py`` (expects ``data.csv`` in the same folder as this file).

**HTML report** (``--html-report``): runs this script with a non-interactive plot backend. Captured ``display()`` / ``plt.show()`` output is grouped by exported ``# In[...]:`` cell boundaries and **woven into the HTML in the same order as** ``assignment3_pipeline.ipynb`` (markdown cell, then that cell’s outputs, …). The notebook file must exist when using ``--html-report``.

  ``python assignment3.py --html-report``
  ``python assignment3.py --html-report /path/to/out.html``

Dependencies: pandas, numpy, matplotlib, seaborn, scikit-learn, torch. Optional: IPython (richer ``display``), ``markdown`` (renders notebook markdown in HTML reports).
"""
...
def _parse_html_report_path() -> Optional[Path]:
    argv = sys.argv[1:]
    for i, a in enumerate(argv):
        if a == "--html-report":
            if i + 1 < len(argv) and not argv[i + 1].startswith("-"):
                return Path(argv[i + 1]).expanduser().resolve()
            return Path(__file__).resolve().parent / "assignment3_report.html"
        if a.startswith("--html-report="):
            return Path(a.split("=", 1)[1]).strip().expanduser().resolve()
    return None


def _strip_html_report_argv() -> None:
    out = [sys.argv[0]]
    skip_next = False
...
_HTML_REPORT_PATH = _parse_html_report_path()
if _HTML_REPORT_PATH is not None:
    import matplotlib as _mpl

    _mpl.use("Agg")
    _strip_html_report_argv()
    _nb_required = Path(__file__).resolve().parent / "assignment3_pipeline.ipynb"
    if not _nb_required.is_file():
        print(
            f"error: --html-report requires the notebook for narrative text: {_nb_required}",
            file=sys.stderr,
        )
        raise SystemExit(1)
...
# Resolve data next to this script so running this file works from any cwd.
_SCRIPT_DIR = Path(__file__).resolve().parent
GDP_DIR = _SCRIPT_DIR
DATA_FILENAME = "data.csv"
DATA_PATH = GDP_DIR / DATA_FILENAME
_NOTEBOOK_PATH = GDP_DIR / "assignment3_pipeline.ipynb"

_html_outputs: list[list[str]] = [[]]
_html_py_block_idx = 0


def _html_enter_cell() -> None:
    """Advance output bucket at each `# In[...]:` boundary (after the first)."""
    global _html_py_block_idx
    if _HTML_REPORT_PATH is None:
        return
    _html_py_block_idx += 1
    _html_outputs.append([])


_HTML_STYLE = """
<style>
body { font-family: system-ui, -apple-system, "Segoe UI", sans-serif; margin: 0 auto; padding: 24px 20px 48px;
  max-width: 1280px; line-height: 1.5; color: #1a202c; background: #f8fafc; }
main.report { background: #fff; border-radius: 8px; padding: 28px 32px 40px;
  box-shadow: 0 1px 3px rgba(0,0,0,.08); }
h1 { color: #1a365d; font-size: 1.75rem; margin-top: 0; }
h2 { color: #2c5282; font-size: 1.35rem; margin-top: 1.75rem; border-bottom: 1px solid #e2e8f0; padding-bottom: 6px; }
.intro { color: #4a5568; font-size: 0.95rem; margin-bottom: 1.25rem; }
.nb-markdown { margin: 1rem 0 1.25rem; padding: 0.35rem 0 0.35rem 1rem; border-left: 3px solid #63b3ed; }
.nb-markdown h1, .nb-markdown h2, .nb-markdown h3, .nb-markdown h4 { margin: 0.85em 0 0.4em; color: #2b6cb0; }
.nb-markdown h2 { border-bottom: none; }
.nb-markdown p { margin: 0.55em 0; }
.nb-markdown ul, .nb-markdown ol { margin: 0.5em 0; padding-left: 1.5em; }
.nb-markdown pre { background: #f7fafc; padding: 12px; overflow-x: auto; font-size: 13px; border-radius: 6px;
  border: 1px solid #e2e8f0; }
.nb-markdown code { background: #edf2f7; padding: 2px 6px; border-radius: 3px; font-size: 0.9em; }
.nb-markdown pre code { background: transparent; padding: 0; border: none; }
.nb-markdown blockquote { margin: 0.6em 0; padding-left: 1em; color: #4a5568; border-left: 3px solid #cbd5e0; }
.nb-markdown table { border-collapse: collapse; font-size: 0.95em; }
.nb-markdown th, .nb-markdown td { border: 1px solid #e2e8f0; padding: 6px 10px; }
.cell-outputs { margin: 0.25rem 0 1.25rem; padding: 4px 0 12px; border-bottom: 1px solid #e2e8f0; }
.cell-outputs:empty { display: none; }
.tbl { border-collapse: collapse; margin: 12px 0; width: 100%; font-size: 13px; }
.tbl th, .tbl td { border: 1px solid #cbd5e0; padding: 6px 8px; text-align: right; }
.tbl th { background: #edf2f7; text-align: center; }
.tbl td.text, .tbl th:first-child { text-align: left; }
.block { margin-bottom: 20px; overflow-x: auto; }
.fig { margin: 16px 0; text-align: center; }
.fig img { max-width: 100%; height: auto; border: 1px solid #e2e8f0; border-radius: 4px; }
</style>
"""


def _html_float_format(x) -> str:
    if isinstance(x, (float, np.floating)):
        return f"{float(x):.6g}"
    return str(x)


def _append_html_table(obj) -> None:
    frag = (
        '<div class="block">'
        + (
            obj.to_html(
                ...
                border=0,
                classes="tbl",
                float_format=_html_float_format,
            )
            if hasattr(obj, "to_html")
            else ("<pre>" + html_module.escape(str(obj)) + "</pre>")
        )
        + "</div>"
    )
    _html_outputs[_html_py_block_idx].append(frag)


def _append_html_figures() -> None:
    ...
    for num in nums:
        ...
        _html_outputs[_html_py_block_idx].append(
            f'<div class="fig"><img src="data:image/png;base64,{b64}" alt="Figure"/></div>'
        )
    plt.close("all")
...
def _plt_show_hook(*args, **kwargs):
    if _HTML_REPORT_PATH is not None:
        _append_html_figures()
    else:
        _orig_plt_show(*args, **kwargs)


plt.show = _plt_show_hook
...
def display(obj, **_kwargs):
    if _HTML_REPORT_PATH is not None:
        _append_html_table(obj)
    elif _ipy_display is not None:
        _ipy_display(obj, **_kwargs)
    else:
        print(obj)


def _markdown_cell_to_html(src: str) -> str:
    if not src.strip():
        return ""
    try:
        import markdown as _md

        body = _md.markdown(src, extensions=["extra", "nl2br"])
    except ImportError:
        body = "<pre>" + html_module.escape(src) + "</pre>"
    return f'<article class="nb-markdown">{body}</article>'


def _finalize_html_report(_script_name: str = Path(__file__).resolve().name) -> None:
    if _HTML_REPORT_PATH is None:
        return
    try:
        ...
        parts = [
            ...
            _HTML_STYLE,
            "<title>GDP Assignment 3 — analysis report</title></head><body>",
            '<main class="report">',
            "<h1>GDP Assignment 3 — analysis report</h1>",
            "<p class=\"intro\"><em>Could not read notebook for layout:</em> "
            + html_module.escape(str(exc))
            + "</p>",
            "</main></body></html>",
        ]
        _HTML_REPORT_PATH.write_text("".join(parts), encoding="utf-8")
        print(f"\n[HTML report written to {_HTML_REPORT_PATH}]", flush=True)
        return
    ...
    parts = [
        ...
        '<main class="report">',
        "<h1>GDP Assignment 3 — analysis report</h1>",
        "<p class=\"intro\">Generated by <code>"
        + html_module.escape(_script_name)
        + "</code> with <code>--html-report</code>. Narrative follows the notebook; tables and figures are interleaved after each code section in notebook order.</p>",
    ]
    code_i = 0
    n_code_nb = sum(1 for c in nb.get("cells", []) if c.get("cell_type") == "code")
    for cell in nb.get("cells", []):
        ct = cell.get("cell_type")
        if ct == "markdown":
            src = "".join(cell.get("source", []))
            frag = _markdown_cell_to_html(src)
            if frag:
                parts.append(frag)
        elif ct == "code":
            parts.append('<section class="cell-outputs">')
            if code_i < len(_html_outputs):
                parts.extend(_html_outputs[code_i])
            parts.append("</section>")
            code_i += 1

    if code_i != len(_html_outputs):
        parts.append(
            "<p class=\"intro\"><em>Layout warning:</em> notebook has "
            f"{n_code_nb} code cells but captured {len(_html_outputs)} output buckets (processed {code_i}).</p>"
        )
    if code_i < len(_html_outputs):
        parts.append('<section class="cell-outputs"><p class="intro"><em>Extra captured outputs:</em></p>')
        for j in range(code_i, len(_html_outputs)):
            parts.extend(_html_outputs[j])
        parts.append("</section>")

    parts.append("</main></body></html>")
    _HTML_REPORT_PATH.parent.mkdir(parents=True, exist_ok=True)
    _HTML_REPORT_PATH.write_text("".join(parts), encoding="utf-8")
    print(f"\n[HTML report written to {_HTML_REPORT_PATH}]", flush=True)


if _HTML_REPORT_PATH is not None:
    atexit.register(_finalize_html_report)
